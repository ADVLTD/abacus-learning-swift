...
class GenerateLinkDropAPIUserTests: XCTestCase {
    ...
    func test_GenerateLinkDrop_WithValidAccountnameAmountAndPrivateKey_ShouldReturnValidResponse() {
        ...
        NearRestAPI.shared.generateLinkDrop(accountName: "headstraitdev2.testnet", amount: "3000000000000000000000000", privateKey: "ed25519:2MJghFQqjgukaERsp1jTtxgXs1iSJQYDd8Ua6W96YQdr32N7Dg5opnHofuBnna9wkaHZKeUARHKEUuj23KdpQ6zp") { result in
            switch result {
            case .success(let response):
                XCTAssertNotNil(response)
                XCTAssertNotNil(response.publicKey)
                XCTAssertNotNil(response.secretKey)
    ...
    func test_GenerateLinkDrop_WithInvalidAccountnameAmountAndPrivateKey_ShouldReturnNilResponse() {
        ...
        NearRestAPI.shared.generateLinkDrop(accountName: "headstraitdev2.testnet", amount: "3000000000000000000000000", privateKey: "ed25519:2MJghFQqjgukaERsp1jTtxgXs1iSJQYDd8Ua6W96YQdr32N7Dg5opnHofuBnna9wkaHZKeUARHKEUuj23KdpQ6z") { result in
            switch result {
            case .success(let response):
                XCTAssertNil(response.amount)
                XCTAssertNil(response.secretKey)
                XCTAssertNil(response.publicKey)
                expectation.fulfill()
            case .failure(let error):
